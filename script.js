    ...
    for (let i = 0; i < bundeslaender.length; i++) {
        ...
        content.innerHTML += `
            <div class="card">
                ...
                <span>Einwohner: ${bundesland['population']} Mio.</span><br>
                <a href="${bundesland['url']}">${bundesland['url']}</a><br>

                <div id="landcontent${i}"> </div>
                <input id="input${i}" type="text" placeholder="Kommentar eingeben"></input><button onclick="addComment(${i})">OK</button><br>
            </div>
        `;

        let landcontent = document.getElementById(`landcontent${i}`);

        for (let j = 0; j < bundesland['comments'].length; j++) {
            const comment = bundesland['comments'][j];
